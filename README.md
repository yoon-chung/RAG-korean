...
- **Best result**: **Public LB MAP 0.9242**, baseline 0.6955 대비 **+33%p**
- **Private set 일반화**: public 점수보다 약 0.01 낮은 패턴 규칙-light 구성이 private set에서 더 잘 일반화 — *generalization gap* 중요 교훈
- **5-Phase 점진적 개선**: classifier evolution → corpus 광범위성 발견 → score threshold + multi-query → 패턴 규칙 처리 → V5 classifier
- **3종 Negative Results**: cross-encoder FT, Qwen3-Reranker 8B 교체, Qwen3-Embedding FT 3-way ensemble
...
| Phase 4 | + force-empty 패턴 규칙 도입 (Standalone Query 패턴 매칭) | 0.9348 | +0.0090 |
| **Phase 5 (best generalization)** | **Classifier-only 재구현 (eval 적합 패턴 규칙 제거)** | **0.9242** | **private set best** |
...
V4 prompt도 *"비과학"으로 판정하던* 질의 다수가 **사실 corpus에 GT 보유**하는 것을 발견:
...
Classifier 끄면 chitchat 질의도 검색됨 → **rerank top-1 score 임계값**으로 일반 메커니즘 도입.
...
- 같은 코드 두 번 돌리면 **±0.005~0.01 LB 변동** → multi-query LLM 비결정성 → **LLM cache 도입**으로 결정성 확보
...
**원인**: GT 라벨 부재 → "다른 문서 = negative" 가정 → BM25/dense 상위는 사실 **정답급 유사 문서**들이라 false negative 오염. Reranker가 "정답에 가까운 docs를 0으로 출력"하도록 잘못 학습됨.
...
### 3) Qwen3-Embedding FT + 3-way Ensemble

외부 GT (ARC train + MMLU validation) 한국어 번역 → corpus 매칭 → MNRL 학습 → 3-way ensemble.

**4-stage pipeline 신규 구현**:
1. `ft01_collect_external_gt.py`: ARC + MMLU 수집 (라이선스 검증, MMLU test/professional_law 제외)
2. `ft02_translate_gt_to_korean.py`: Solar Pro batch 번역 (2,724 → 2,714 성공, 99.6%)
3. `ft03_match_gt_to_corpus.py`: bge-m3 + reranker 기반 매칭 (1,365 pair, threshold 0.2)
4. `ft04_train_qwen_embed_mnrl.py`: Qwen3-Embedding-0.6B full FT (MNRL, in-batch negative)

**결과**: 3-way ensemble LB **0.9212** (−0.0046 vs tau002 0.9258).

**원인**:
- top1 retrieval은 86% 안정 (FT가 큰 손상 없음)
- 하지만 ensemble diversity 신호가 약함 — bge-m3 단독 대비 의미 있는 추가 signal 못 만듦
- corpus 광범위성 문제 (Phase 2에서 이미 확인) 가 root cause

**학습**: ensemble diversity는 모델 다양화로 자동 확보되지 않음. 데이터·도메인 차이가 핵심.
...
```bash
python ft01_collect_external_gt.py    # ARC + MMLU 수집
python ft02_translate_gt_to_korean.py # Solar Pro 번역
python ft03_match_gt_to_corpus.py     # bge로 corpus 매칭
python ft04_train_qwen_embed_mnrl.py  # Qwen3-Embedding FT
```
...
│   ├── rag_exp-*.py              # 실험 wrapper (각 실험의 default config 고정)
│   ├── ft01-04_*.py              # Qwen3-Embedding FT 4-stage pipeline
...
│   ├── ft_run_*.sh               # nohup 배치 실행 스크립트
│   ├── notes/                    # 단계별 실험 기록 (수십 개 markdown)
│   └── .env.template
├── EDA/                          # 초기 EDA 노트북
└── README.md (this file)
```
