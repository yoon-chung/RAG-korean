...
class Runtime:
    def __init__(self, cfg: Config):
        ...
        # Rescue reranker (empty 케이스 2차 검증용)
...
def retrieve(rt: Runtime, query: str) -> List[Tuple[str, float, dict]]:
    ...
    if cfg.score_threshold > 0 and reranked and reranked[0][1] < cfg.score_threshold:
        # --- Rescue reranker: empty 판정된 케이스를 FT reranker로 2차 검증 ---
        if rt.rescue_reranker is not None and candidates:
            rescue_pairs = [(query, c[2]["content"]) for c in candidates[:cfg.topk_rerank]]
            rescue_scores = rt.rescue_reranker.predict(rescue_pairs)
            rescue_top = max(rescue_scores)
            if rescue_top > cfg.rescue_threshold:
                rescored = [(c[0], float(s), c[2]) for c, s in zip(candidates[:cfg.topk_rerank], rescue_scores)]
                rescored.sort(key=lambda x: x[1], reverse=True)
                print(f"  [RESCUE] query='{query[:40]}' rescued with score={rescue_top:.4f}")
