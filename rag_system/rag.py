...
실험 순서와 명령 모음은 notes/00_experiment_plan.md 참고.
"""
...
# FAISS는 Solar 4096d 풀 차원 사용을 위한 인메모리 dense store (EXP-02-003-04-faiss)
...
class Config:
    ...
    classify_prompt: str = "v4"           # v1|v2|v3|v4|v5 (V5: recall-bias + corpus scope)

    # Models
    embed_provider: str = "local"         # local (SentenceTransformer) | solar (Upstage API) (EXP-02-003-04)
    ...
    topk_final: int = 3
    rrf_k: int = 60
    score_threshold: float = 0.0          # rerank top-1 score < 이 값이면 topk=[] (EXP-02-003-01)
    use_hyde: bool = False                # HyDE: 가상 답변 문서를 dense 쿼리로 사용 (EXP-02-003-02)
    rrf_weight_sparse: float = 1.0        # sparse RRF 가중치 (EXP-02-003-03)
    rrf_weight_dense: float = 1.0         # dense RRF 가중치 (EXP-02-003-03)
    extra_embed_model: str = ""           # 2차 dense retriever (ensemble, FAISS 전용). 비우면 off (EXP-02-003-07)
    ...
    base_submission: str = ""             # 기존 submission.csv 경로. non-empty topk는 보존, empty만 재처리 (EXP-02-003-11)
    rescue_filter_classifier: bool = False  # empty 재처리 전 classifier 실행. is_science=false면 empty 유지
    rescue_filter_sq_pattern: bool = False  # base empty의 SQ가 BIOGRAPHY_SQ_PATTERNS 매치 시 empty 유지 (EXP-02-003-12)
    force_empty_sq_pattern: bool = False  # base non-empty의 SQ가 FORCE_EMPTY_SQ_PATTERNS 매치 시 강제 empty (EXP-02-003-14)

    # Multi-query inference: SQ 1개 → N개 paraphrase 생성 → 각각 retrieve → RRF fuse
    # SQ 한 번의 운에 의존하지 않고 reranker score 안정화 (EXP-07)
...
def parse_args() -> Config:
    ...
    p.add_argument("--classify-prompt", choices=["v1", "v2", "v3", "v4", "v5"], default="v4",
                   help="LLM 분류기 프롬프트 버전 (default: v4). v5: recall-bias + corpus scope")

    # Models
    p.add_argument("--embed-provider", choices=["local", "solar"], default=None,
                   help="local (SentenceTransformer) | solar (Upstage API) (EXP-02-003-04)")
    ...
    p.add_argument("--score-threshold", type=float, default=0.0,
                   help="rerank top-1 score < 이 값이면 해당 질의 topk=[] (EXP-02-003-01)")
    p.add_argument("--hyde", action="store_true",
                   help="HyDE: LLM이 생성한 가상 답변 문서를 dense 쿼리로 사용 (EXP-02-003-02)")
    p.add_argument("--rrf-weight-sparse", type=float, default=1.0,
                   help="sparse RRF 가중치 (EXP-02-003-03)")
    p.add_argument("--rrf-weight-dense", type=float, default=1.0,
                   help="dense RRF 가중치 (EXP-02-003-03)")
    ...
    p.add_argument("--base-submission", default="",
                   help="기존 submission.csv 경로. non-empty topk는 그대로 보존, empty만 재처리 (EXP-02-003-11)")
    ...
    cfg.classify_prompt = args.classify_prompt
...
class Runtime:
    def __init__(self, cfg: Config):
        ...
        if cfg.embed_provider == "solar":
            # Upstage Solar embedding API + FAISS 인메모리 dense store (EXP-02-003-04-faiss)
        ...
        # Extra (2nd) dense retriever for ensemble — always FAISS (EXP-02-003-07)
        ...
        if cfg.use_reranker:
            if cfg.reranker_model.startswith("Qwen/Qwen3-Reranker"):
                # Qwen3-Reranker는 causal LM의 yes/no 토큰 확률로 점수 산출 (EXP-02-003-08)
        ...
        if cfg.rescue_reranker:
            if cfg.rescue_reranker.startswith("Qwen/Qwen3-Reranker"):
                # Qwen3-Reranker를 rescue로 사용 (EXP-02-003-10)
                try:
                    from transformers import AutoTokenizer, AutoModelForCausalLM
                    import torch
        ...
        # Base submission (EXP-02-003-11): non-empty topk는 그대로 보존, empty만 재처리
    ...
    def embed(self, sentences, model_type: str = "passage", use_extra: bool = False):
        ...
        if not use_extra and self.cfg.embed_provider == "solar":
            # Solar는 query/passage 모델이 분리됨 (EXP-02-003-04)
            # FAISS 사용으로 truncation 없이 4096d 풀 차원 그대로 사용 (EXP-02-003-04-faiss)
...
def extra_dense_search(rt: Runtime, query: str, size: int):
    """2차 dense retriever (ensemble용, FAISS 전용). EXP-02-003-07"""
...
def rrf_fuse(result_lists: List[list], k: int = 60, weights: Optional[list] = None):
    # --- 원래 코드 (EXP-02-003-02까지 사용, 필요 시 복구) ---
    # scores, sources = {}, {}
    ...
    # --- Weighted RRF 적용 버전 (EXP-02-003-03) ---
    # weights=None (default) → 모든 리스트 동일 가중치 (기존 동작과 동일)
...
def qwen3_rerank_batch(rt: Runtime, query: str, docs: List[str], batch_size: int = 4) -> List[float]:
    """Qwen3-Reranker score: P(yes) / (P(yes) + P(no))  (EXP-02-003-08)
...
def qwen3_rescue_rerank_batch(rt: Runtime, query: str, docs: List[str], batch_size: int = 4) -> List[float]:
    """Rescue 경로용 Qwen3-Reranker. qwen3_rerank_batch와 동일한 로직, rescue 전용 속성 참조 (EXP-02-003-10)"""
    import torch
    contents = [
        f"<Instruct>: {QWEN3_RR_INSTRUCTION}\n<Query>: {query}\n<Document>: {doc}"
        for doc in docs
...
def retrieve(rt: Runtime, query: str) -> List[Tuple[str, float, dict]]:
    ...
    # --- Multi-query (EXP-07): SQ → N개 paraphrase 생성, 각 query로 retrieve 후 RRF fuse ---
    ...
    if cfg.multi_query and mode == "hybrid":
        ...
        for q in queries:
            ...
            # 3-way ensemble: extra dense retriever (EXP-02-003-07)
        ...
        # --- HyDE: dense 검색용 가상 답변 문서 생성 (EXP-02-003-02) ---
    ...
    #     return rerank_candidates(rt, query, candidates, cfg.topk_final)
    # return candidates[:cfg.topk_final]

    # --- score threshold filter 적용 버전 (EXP-02-003-01) ---
    ...
    if cfg.score_threshold > 0 and reranked and reranked[0][1] < cfg.score_threshold:
        ...
        # Cross-encoder (FT) 또는 Qwen3-Reranker 둘 다 지원 (EXP-02-003-10)
...
CLASSIFY_SYSTEM_V5 = """너는 과학·상식 지식 검색 시스템의 질의 분석기다.

## 시스템 컨텍스트
이 시스템은 약 4,200개의 한국어 지식 문서를 검색한다.
문서 출처: MMLU + ARC 한국어 번역본.
포함 주제: 자연과학, 의학, 공학, 기초 컴퓨터과학, 역사·문화 일부, 생활상식·인물 일부.
즉 "학교 교과서·일반 지식책에 실릴 만한" 거의 모든 사실 기반 질문이 검색 대상이다.

## 판정 우선순위 (recall 우선)
검색이 도움될 가능성이 조금이라도 있으면 is_science=true.
is_science=false 는 아래 명시 카테고리에 명백히 들어갈 때만 적용:

### 비과학 (검색 차단) — 좁게 정의
- 인사·감탄·칭찬 ("안녕", "고마워", "넌 똑똑하구나")
- AI 자기지칭 ("너는 누구야", "너 잘하는 게 뭐야", "넌 모르는 게 있니")
- 위로 요청·감정 토로 ("힘들다", "우울해", "외로워", "지친다")
- 개인 취향·일정·연애·인간관계 상담 ("내 친구가...", "썸타는 사람이...")
- 단순 메타 대화 ("그건 알겠고", "다음 질문")

### 위 카테고리에 명시적으로 들어가지 않는 모든 입력 = is_science=true
- 애매하면 true (검색 후단의 reranker score가 신뢰도로 걸러냄)
- 인물·역사·생활상식·실용지식 모두 true
- 코드·프로그래밍 질문도 true (CS 이론·실습)

## Standalone Query 작성 규칙
is_science=true 일 때만 작성. 멀티턴 대화의 마지막 질문을 독립형 한국어로 재작성.
- 지시대명사("그것", "이 사건")는 이전 발화로 치환
- 핵심 키워드 포함한 한 문장
is_science=false 면 빈 문자열("").
...
# 분류기 프롬프트 레지스트리. CLI --classify-prompt 로 선택 (default v4)
CLASSIFY_PROMPT_REGISTRY = {
    "v1": CLASSIFY_SYSTEM_V1,
    "v2": CLASSIFY_SYSTEM_V2,
    "v3": CLASSIFY_SYSTEM_V3,
    "v4": CLASSIFY_SYSTEM_V4,
    "v5": CLASSIFY_SYSTEM_V5,
}
CLASSIFY_SYSTEM = CLASSIFY_SYSTEM_V4  # 기본값. 런타임에 cfg.classify_prompt 로 선택됨

# AI 자기지칭 chitchat SQ 패턴 (classifier=none + τ 하향 시 chitchat이 retrieval 통과하는 문제 보정)
# rewrite_only는 모든 질의에 SQ 생성하므로 후처리 필터 필요 (EXP-02-003-07 Phase 8)
...
# 인물·전기류 SQ 패턴 — rescue 후보에서 제외 (EXP-02-003-12)
...
# 비과학 SQ 패턴 — base non-empty 저신뢰 케이스 강제 empty
# tau002 public 저신뢰 non-empty 중 비과학 주제 타겟 (line<110 public 집합)
#   - (EXP-14) "힘든 상황" (218/276, score 0.084): 감정·웰니스 → chitchat 확정 (+0.009)
#   - (EXP-15) "라틴 역사" (212, score 0.034): 과학 GT hit (-0.009) → 제거
#   - (EXP-16) "통학 버스" (81, score 0.054): 과학 GT hit (-0.009) → 제거
# 교훈: bge 저신뢰 ≠ chitchat. 단건 LB 검증만 신뢰 가능.
...
# --- HyDE (EXP-02-003-02; EXP-07에서 도메인 중립화) ---
...
def classify_and_rewrite(rt: Runtime, messages):
    system_prompt = CLASSIFY_PROMPT_REGISTRY.get(rt.cfg.classify_prompt, CLASSIFY_SYSTEM)
    content = cached_chat_completion(
        rt,
...
def generate_hyde_doc(rt: Runtime, query: str) -> str:
    """HyDE: 질의에 대한 가상 답변 문서 생성. dense retrieval 쿼리로 사용됨. (EXP-02-003-02)"""
    try:
        content = cached_chat_completion(
            rt,
            messages=[
...
def eval_rag(rt: Runtime):
    ...
    with open(cfg.eval_path) as f, open(submission_path, "w") as of:
        for idx, line in enumerate(f):
            ...
            # Base submission 분기 (EXP-02-003-11):
            ...
            if eval_id in rt.base_submission:
                ...
                if base.get("topk"):
                    # Force-empty: base non-empty이지만 SQ가 감정 발화 패턴이면 topk 비움 (EXP-02-003-14)
